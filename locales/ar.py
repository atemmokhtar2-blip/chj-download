STRINGS = {
    "choose_language": (
        "🚀 <b>X</b>\n\n"
        "⚡ تحميل سريع\n"
        "🎬 فيديو وصوت وصور\n"
        "🎵 MP3 عالي الجودة\n"
        "📹 +13 منصة مدعومة\n\n"
        "<b>المنصات المدعومة:</b>\n"
        "• YouTube  • TikTok  • Instagram\n"
        "• Facebook  • Twitter/X  • Threads\n"
        "• Reddit  • Pinterest  • Snapchat\n"
        "• Vimeo  • Dailymotion  • SoundCloud\n"
        "• Telegram\n\n"
        "اختر لغتك:"
    ),
    "language_set": "✅ تم ضبط اللغة على العربية!",

    "welcome": (
        "🚀 <b>مرحباً بك في أقوى بوت تحميل على تلغرام! ✨</b>\n\n"
        "➜ يمكنك تحميل الفيديوهات والصور من (تيك توك، إنستقرام، يوتيوب، فيسبوك) وأكثر من 13 منصة أخرى بضغطة واحدة وبدون علامة مائية! 🔥\n\n"
        "📎 <b>أرسل الرابط الآن وابدأ التحميل فوراً!</b>"
    ),
    "welcome_back": (
        "🎬 ➤ <b>مرحباً بك مجدداً ✨</b>\n\n"
        "➜ أرسل رابط أي فيديو أو صورة للتحميل مباشرة بأعلى جودة."
    ),

    "menu_download":     "📥 تحميل",
    "menu_profile":      "👤 حسابي",
    "menu_favorites":    "⭐ المفضلة",

    "menu_settings":     "⚙️ الإعدادات",
    "menu_help":         "ℹ️ المساعدة",

    "menu_achievements": "🏆 الإنجازات",
    "menu_wheel":        "🎰 عجلة الحظ",

    "send_url_prompt": "📎 <b>من فضلك أرسل الرابط الذي تريد تحميله مباشرة بأعلى جودة:</b>",

    "join_channel": (
        "📢 <b>الاشتراك في القناة مطلوب</b>\n\n"
        "يجب عليك الانضمام إلى قناتنا @h7h7h7Updates لاستخدام هذا البوت.\n\n"
        "👇 اضغط على الزر أدناه للانضمام، ثم اضغط ✅ تحقق."
    ),
    "join_button":    "📢 انضم للقناة",
    "verify_button":  "✅ تحقق",
    "not_subscribed": "🚫 لم تنضم للقناة بعد. يرجى الانضمام أولاً ثم اضغط ✅ تحقق.",
    "subscribed_ok":  "✅ تم التحقق من الاشتراك! يمكنك الآن استخدام البوت.",

    "analyzing":       "🔍 <b>تحليل الرابط...</b>\n\n░░░░░░░░░░ 0%",
    "unsupported_url": (
        "⚠️ <b>منصة غير مدعومة</b>\n\n"
        "المنصات المدعومة: YouTube، TikTok، Instagram، Facebook، Twitter/X،\n"
        "Threads، Reddit، Pinterest، Snapchat، Vimeo، Dailymotion، SoundCloud، Telegram."
    ),
    "invalid_url": (
        "❌ <b>رابط غير صالح</b>\n\n"
        "يرجى إرسال رابط صحيح من منصة مدعومة."
    ),
    "analysis_failed": (
        "❌ <b>فشل التحليل</b>\n\n"
        "تعذّر قراءة الرابط. قد يكون خاصاً أو غير متاح. يرجى تجربة رابط آخر."
    ),

    "video_info": (
        "🎬 <b>معلومات الوسائط</b>\n\n"
        "📌 <b>العنوان:</b>\n{title}\n\n"
        "👤 <b>الناشر:</b> {uploader}\n"
        "⏱ <b>المدة:</b> {duration}\n"
        "📺 <b>المنصة:</b> {platform}\n"
        "📁 <b>النوع:</b> {media_type}\n\n"
        "👇 <b>اختر خياراً:</b>"
    ),
    "download_video": "📹 تحميل فيديو",
    "download_audio": "🎵 تحميل MP3",
    "download_image": "🖼 تحميل صورة",
    "download_album": "📁 تحميل ألبوم",
    "best_quality":   "🏆 أفضل جودة",
    "info_button":    "ℹ️ معلومات",
    "cancel_button":  "❌ إلغاء",
    "cancelled":      "❌ تم الإلغاء.",

    "not_enough_points": (
        "❌ <b>نقاط غير كافية</b>\n\n"
        "تحتاج إلى نقطة واحدة على الأقل للتحميل.\n"
        "اكسب نقاطاً عبر:\n"
        "• 🎁 المكافأة اليومية (/daily)\n"
        "• 🎰 عجلة الحظ (/wheel)\n"

    ),
    "not_enough_points_short": "❌ نقاط غير كافية. استخدم /daily أو /wheel لكسب المزيد.",

    "downloading":     "⬇️ <b>جاري التحميل</b>\n\n{bar} {percent}%\n\n📦 {downloaded} / {total}\n⚡ {speed}\n⏱ الوقت المتبقي: {eta}",
    "processing":      "🎵 جارٍ معالجة الصوت...",
    "uploading":       "📤 <b>رفع الملف...</b>\n\n██████████ 100%",
    "completed":       "✅ <b>اكتمل التحميل!</b>",
    "completed_detail": (
        "✅ <b>اكتمل التحميل</b>\n\n"
        "🎬 <b>المنصة:</b> {platform}\n"
        "🎞 <b>الجودة:</b> {quality}\n"
        "📦 <b>الحجم:</b> {size}\n\n"
        "شكراً لاستخدامك X. 🙏"
    ),
    "from_cache":      "⚡ <b>تم الإرسال فوراً من الذاكرة المؤقتة!</b>",
    "download_failed": "❌ فشل التحميل. يرجى المحاولة مجدداً.",
    "file_too_large":  "❌ الملف كبير جداً للإرسال عبر تيليغرام (الحد الأقصى {max_mb}MB).",

    "rate_limit": "⏳ يرجى الانتظار {seconds} ثانية قبل التحميل مجدداً.",
    "queue_full":  "⏳ قائمة التحميل ممتلئة. يرجى المحاولة بعد لحظة.",

    "profile_text": (
        "👤 <b>الملف الشخصي</b>\n\n"
        "🆔 <b>User ID:</b> <code>{user_id}</code>\n"
        "⭐ <b>النقاط:</b> {points}\n"
        "🏅 <b>المستوى:</b> {level}\n"
        "📥 <b>عدد التحميلات:</b> {downloads}\n"
        "   📹 فيديو: <b>{video_downloads}</b>  🎵 صوت: <b>{audio_downloads}</b>\n"

        "🏆 <b>الإنجازات:</b> {achievements}\n"
        "📅 <b>تاريخ الانضمام:</b> {join_date}"
    ),
    "profile_history_btn":   "📜 السجل",
    "profile_favorites_btn": "⭐ المفضلة",
    "profile_wheel_btn":     "🎰 عجلة الحظ",
    "profile_back_btn":      "🔙 رجوع",

    "history_title": "📜 <b>التحميلات الأخيرة</b>\n\n",
    "history_empty": "📭 لم تقم بأي تحميل بعد.",
    "history_item":  "• <b>{title}</b>\n  📺 {platform} | 🎬 {quality} | 📅 {date}\n",



    "points_text": (
        "⭐ <b>نقاطك: {points}</b>\n\n"
        "💡 كل تحميل يكلف نقطة واحدة.\n"
        "اكسب المزيد عبر /daily و/wheel.\n\n"
        "🎁 <b>المكافآت المتاحة:</b>\n"
        "{rewards}"
    ),
    "redeem_button":     "🎁 استبدال {cost} نقطة — {name}",
    "redeem_success":    "✅ تم الاستبدال: {name}! استمتع بمكافأتك.",
    "redeem_not_enough": "❌ نقاط غير كافية. تحتاج {cost} نقطة (لديك {have}).",

    "daily_claimed": "🎁 تم المطالبة بالمكافأة اليومية! +{points} نقطة.\n\n⭐ المجموع: {total} نقطة",
    "daily_already": "⏳ لقد طالبت بمكافأة اليوم. عد بعد {hours}س {minutes}د!",

    # عجلة الحظ
    "wheel_spin": "🎰 <b>عجلة الحظ</b>\n\nاستدر لتكسب نقاطاً إضافية!\n\n🎫 الجوائز المتاحة: 1، 2، 3، 5، أو 10 نقاط.\n\nاضغط للإدارة:",
    "wheel_spin_btn":  "🎰 أدِر العجلة!",
    "wheel_spinning":  "🎰 جاري الإدارة...",
    "wheel_win":       "🎉 <b>ربحت {points} نقطة!</b>\n\n⭐ المجموع: {total} نقطة\n\nعد بعد 24 ساعة للدورة التالية!",
    "wheel_cooldown":  "⏳ لقد أدرت العجلة اليوم! عد بعد {hours}س {minutes}د.",
    "wheel_jackpot":   "🎊 <b>جاكبوت!</b> ربحت 10 نقاط!\n\n⭐ المجموع: {total} نقطة 🔥",


    "weekly_tab":        "📅 أسبوعي",
    "monthly_tab":       "🗓 شهري",

    "achievements_title":   "🏆 <b>إنجازاتك</b>\n\n",
    "achievement_unlocked": "🏆 تم فتح إنجاز: {name}!",

    "share_bot":      "📤 مشاركة البوت",
    "share_bot_text": "🎉 <b>شكراً للتحميل!</b>\n\nشارك هذا البوت مع أصدقائك واكسب نقاطاً! 🎁",

    "settings_text": (
        "⚙️ <b>الإعدادات</b>\n\n"
        "🌐 اللغة: <b>{language}</b>\n"
        "🎥 الجودة الافتراضية: <b>أفضل جودة</b>\n"
        "🎵 صيغة الصوت: <b>MP3</b>\n"
        "🔔 الإشعارات: <b>مفعّلة</b>"
    ),
    "settings_lang_btn": "🌐 تغيير اللغة",

    "help_text": (
        "❓ <b>المساعدة والأوامر</b>\n\n"
        "<b>📥 كيفية التحميل:</b>\n"
        "أرسل أي رابط مدعوم مباشرةً للبوت.\n\n"
        "<b>💡 نظام النقاط:</b>\n"
        "• تبدأ بـ 10 نقاط مجانية\n"
        "• كل تحميل يكلف نقطة واحدة\n"
        "• اكسب عبر /daily و/wheel\n\n"
        "<b>📋 الأوامر:</b>\n"
        "/profile — ملفك الشخصي ومستواك\n"

        "/points — النقاط والمكافآت\n"
        "/daily — المكافأة اليومية (+5 نقاط)\n"
        "/wheel — عجلة الحظ\n"
        "/favorites — المفضلة\n"

        "/achievements — إنجازاتي\n\n"
        "<b>✅ المنصات المدعومة:</b>\n"
        "YouTube • TikTok • Instagram • Facebook\n"
        "Twitter/X • Threads • Reddit • Pinterest\n"
        "Snapchat • Vimeo • Dailymotion • SoundCloud • Telegram"
    ),

    "favorites_title":  "⭐ <b>المفضلة</b>\n\n",
    "favorites_empty":  "📭 لا توجد مفضلات بعد.",
    "favorite_added":   "⭐ تمت الإضافة إلى المفضلة!",
    "favorite_removed": "🗑 تمت الإزالة من المفضلة.",
    "add_favorite":     "⭐ حفظ",
    "remove_favorite":  "🗑 إزالة",

    "community_goals": (
        "🎯 <b>أهداف المجتمع</b>\n\n"
        "إجمالي المستخدمين: {total}\n\n"
        "{goals}"
    ),
    "goal_item_done":   "✅ {threshold} مستخدم — {reward}\n",
    "goal_item_next":   "🔜 {threshold} مستخدم — {reward} (يتبقى {remaining})\n",
    "goal_item_future": "🔒 {threshold} مستخدم — {reward}\n",



    "session_no_url":   "❌ لم يُعثر على رابط في الجلسة.",
    "session_expired":  "⏳ انتهت الجلسة. يرجى إرسال الرابط مجدداً.",
    "no_db_user":       "⚠️ يرجى استخدام /start أولاً.",

    "first_download_reward": (
        "🎉 <b>تهانينا على أول تحميل!</b>\n\n"
        "⭐ <b>+10 نقاط إضافية</b> أضيفت إلى حسابك!\n\n"
        "استمر في التحميل للوصول إلى المستوى التالي! 🚀"
    ),

    "top_downloads_title": "🔥 <b>الأكثر تحميلاً</b>\n\n",
    "top_downloads_item":  "{rank}. <b>{title}</b>\n   📥 {count} تحميل | 📺 {platform}\n\n",
    "top_downloads_empty": "📭 لا توجد تحميلات مسجلة بعد.",

    "maintenance_text": (
        "🚧 <b>البوت في وضع الصيانة</b>\n\n"
        "X تحت الصيانة مؤقتاً.\n"
        "يرجى المحاولة لاحقاً. 🙏"
    ),

    # ─── Admin ───
    "admin_welcome": (
        "👑 <b>لوحة تحكم الأدمن</b>\n\n"
        "أهلاً بك في لوحة التحكم. اختر عملية:\n"
    ),
    "admin_panel_btn": "📊 الإحصائيات",
    "admin_users_btn": "👥 المستخدمون",
    "admin_broadcast_btn": "📢 بث",
    "admin_system_btn": "🖥 النظام",
    "admin_search_btn": "🔍 بحث",
    "admin_maintenance_btn": "🚧 الصيانة",

    "admin_vip_btn": "⭐ VIP",
    "admin_points_btn": "💰 نقاط",
    "admin_ban_btn": "🚫 حظر",
    "admin_no_users": "📭 لا يوجد مستخدمون.",
    "admin_users_title": "👥 <b>المستخدمون</b>",
    "admin_user_not_found": "❌ لم يتم العثور على المستخدم.",
    "admin_search_prompt": "🔍 <b>ابحث عن مستخدم:</b>\n\nأرسل ID أو اسم المستخدم:",
    "admin_search_results": "🔍 <b>نتائج البحث:</b>",
    "admin_vip_prompt": "⭐ <b>تعيين VIP:</b>\n\nأرسل: <code>user_id days</code>\nمثال: 123456 30",
    "admin_vip_usage": "❌ الاستخدام: <code>user_id days</code>\nمثال: 123456 30",
    "admin_vip_set": "✅ تم تعيين VIP للمستخدم <code>{user_id}</code> لمدة <b>{days}</b> يوم.",
    "admin_points_prompt": "💰 <b>تعديل نقاط:</b>\n\nأرسل: <code>user_id amount</code>\n(قيمة موجبة للإضافة، سالبة للحذف)\nمثال: 123456 10",
    "admin_points_usage": "❌ الاستخدام: <code>user_id amount</code>\nمثال: 123456 10",
    "admin_points_set": "✅ تم تعديل نقاط المستخدم <code>{user_id}</code>. النقاط الجديدة: <b>{new_total}</b>",
    "admin_points_negative_error": "❌ لا يمكن جعل النقاط سالبة.",
    "admin_ban_prompt": "🚫 <b>حظر/فك حظر:</b>\n\nأرسل ID أو @username:",
    "admin_banned": "🚫 تم حظر المستخدم!",
    "admin_unbanned": "✅ تم فك حظر المستخدم!",
    "admin_broadcast_intro": (
        "📢 <b>إرسال بث</b>\n\n"
        "اضغط الزر أدناه ثم أرسل الرسالة التي تريد إرسالها لجميع المستخدمين."
    ),
    "admin_broadcast_send": "📤 إرسال رسالة بث",
    "admin_broadcast_message_prompt": "📢 <b>أرسل الرسالة التي تريد بثها:</b>",
    "admin_broadcast_progress": "📢 <b>جاري إرسال البث...</b>\n\n📤 تم: {done} / {total}",
    "admin_broadcast_done": "✅ <b>اكتمل البث!</b>\n\n📤 إجمالي: {total}\n✅ نجح: {success}\n❌ فشل: {failed}",
    "admin_broadcast_cancel": "❌ تم إلغاء البث.",
    "admin_maintenance_text": (
        "🚧 <b>وضع الصيانة</b>\n\n"
        "الحالة الحالية: <b>{status}</b>"
    ),
    "admin_maintenance_on": "تشغيل",
    "admin_maintenance_off": "إيقاف",
    "maintenance_on": "🔴 تم تفعيل وضع الصيانة!",
    "maintenance_off": "🟢 تم إيقاف وضع الصيانة!",
    "maintenance_on_status": "🔴 مفعّل",
    "maintenance_off_status": "🟢 متوقف",
    "admin_refstats_title": "📊 <b>إحصائيات الدعوات</b>",
    "admin_refstats_total": "إجمالي الدعوات",
    "admin_refstats_completed": "مكتملة",
    "admin_refstats_pending": "معلقة",
    "admin_top_referrers": "أفضل المحيلين",
    "admin_referral_invites": "دعوة",
    "yes": "نعم",
    "no": "لا",
    "profile_admin_badge": "\n👑 <b>مسؤول</b>",

    "banned": "🚫 لقد تم حظرك من استخدام هذا البوت.",

    "admin_panel": (
        "👑 <b>لوحة تحكم الأدمن</b>\n\n"
        "👥 إجمالي المستخدمين: <b>{total_users}</b>\n"
        "🟢 نشط اليوم: <b>{active_today}</b>\n\n"
        "📥 تحميلات اليوم: <b>{downloads_today}</b>\n"
        "📅 تحميلات الأسبوع: <b>{downloads_week}</b>\n"
        "🗓 تحميلات الشهر: <b>{downloads_month}</b>\n\n"
        "🎬 يوتيوب: <b>{youtube}</b> | 📘 فيسبوك: <b>{facebook}</b> | 📌 بنترست: <b>{pinterest}</b>\n\n"
        "💾 نجاح الذاكرة: <b>{cache_hits}</b>\n"
        "⭐ نقاط موزعة: <b>{points_issued}</b>\n"
        "🟢 حالة البوت: <b>متصل</b>"
    ),

    "system_monitor": (
        "🖥 <b>مراقب النظام</b>\n\n"
        "🖥 CPU: <b>{cpu}%</b>\n"
        "🧠 RAM: <b>{ram_used} MB / {ram_total} MB</b> ({ram_pct}%)\n"
        "💿 Disk: <b>{disk_used} GB / {disk_total} GB</b> ({disk_pct}%)\n"
        "💽 حجم القاعدة: <b>{db_size} KB</b>\n"
        "⏱ مدة التشغيل: <b>{uptime}</b>\n"
        "🕐 آخر نبضة: <b>{heartbeat_age}</b>"
    ),

    "admin_user_card": (
        "👤 <b>معلومات المستخدم</b>\n\n"
        "🆔 ID: <code>{user_id}</code>\n"
        "👤 الاسم: <b>{name}</b>\n"
        "🔖 اليوزر: {username}\n"
        "⭐ النقاط: <b>{points}</b>\n"
        "📥 التحميلات: <b>{downloads}</b>\n"
        "👥 الدعوات: <b>{referrals}</b>\n"
        "🚫 محظور: <b>{is_banned}</b>\n"
        "📅 انضم: {join_date}"
    ),



    "status_text": (
        "📊 <b>حالة البوت</b>\n\n"
        "🕒 الوقت الحالي: {current_time}\n"
        "⏱ مدة التشغيل: {uptime}\n"
        "📦 التحميلات النشطة: {active_tasks}\n"
        "👥 المستخدمين النشطين: {active_users}"
    ),

}
